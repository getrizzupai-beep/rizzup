...
function _updateNavUI(profile) {
  ...
  const genderBadge = document.getElementById('nav-gender-badge');  // ← NEW

  const firstName = profile.name?.split(' ')[0] || 'User';
  const plan = (profile.plan || 'free').toUpperCase();
  const gender = profile.gender || 'male';  // ← GET GENDER
  const genderIcon = gender === 'female' ? '👩' : '👨';  // ← GENDER ICON

  if (planBadge) planBadge.textContent = plan;
  if (userName) userName.textContent = firstName;
  if (userNameDash) userNameDash.textContent = firstName;
  if (genderBadge) genderBadge.textContent = genderIcon;  // ← SHOW GENDER
}
